<script lang="ts">
	...
	import FloatingOrbs from '$lib/FloatingOrbs.svelte';
	import MouseFollowGradient from '$lib/MouseFollowGradient.svelte';
...
<style>
	...
	/* Impressive Button Shine Effect */
	.btn {
		position: relative;
		overflow: visible;
	}
	...
	/* Impressive Hero Text Reveal */
	@keyframes heroTextGlitch {
		0%, 100% {
			text-shadow: 0 0 20px rgba(100, 200, 255, 0), 0 0 0 rgba(100, 200, 255, 0);
		}
		50% {
			text-shadow: 0 0 30px rgba(100, 200, 255, 0.3), 0 0 10px rgba(100, 200, 255, 0.5);
		}
	}

	.subtitle {
		animation: heroTextGlitch 4s ease-in-out infinite;
	}
	...
	.experience-card-modern:hover::after,
	.bio-card-3d:hover::after {
	...
	/* Enhanced Card Tilt Shadow */
	.card-inner-3d:hover,
	.contact-card-3d:hover {
		filter: drop-shadow(0 20px 40px rgba(100, 200, 255, 0.15)) drop-shadow(0 0 20px rgba(100, 200, 255, 0.1));
	}
	...
	.interest-number {
		animation: numberFlip 0.8s cubic-bezier(0.68, -0.55, 0.265, 1.55);
	}

	/* Impressive Link Hover Trail */
	...
	.floating-item {
		animation: floatImproved 6s ease-in-out infinite;
	}
	...
	/* Impressive Background Texture Overlay */
	main::before {
		content: '';
		position: fixed;
		top: 0;
